    ...
    <main class="flex-1 min-w-0 p-4 pt-20 md:p-7 md:pt-7">
        ...
        <?php if ($today_milk > 0 && $remaining_milk <= 0): ?>
            <div class="mb-5 bg-red-100 border border-red-300 text-red-800 px-4 py-3 rounded-xl flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm">All <?= number_format($today_milk, 1) ?> litres from today's production have been sold.</p>
                </div>
            </div>
        <?php endif; ?>
        ...
        <div class="grid md:grid-cols-3 gap-6 mb-7">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
                ...
                <p class="text-slate-500 mt-2">Remaining Milk</p>
                <span class="bg-green-100 text-green-700 text-sm px-3 py-1 rounded-full font-medium inline-block mt-2">
                    NRM value: KSh <?= number_format(max(0, $remaining_milk * $milk_price), 2) ?>
                </span>
            </div>
                        ...
                        <?php else: ?>
                            <?php foreach ($milk_sales as $sale): ?>
                                <tr class="border-t hover:bg-slate-50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <form method="POST" onsubmit="return confirm('Delete this sale record?')" style="display:inline;">
...
<div id="saleModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-50">
    <div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl p-7 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-6">
            ...
            <button onclick="closeSaleModal()" class="text-slate-400 hover:text-slate-600">
        ...
        <form method="POST" class="space-y-5">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="customer_name"
                           placeholder="e.g. John Kamau" maxlength="100" required
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 mb-2">Available today: <strong><?= number_format($remaining_milk, 1) ?> L</strong></p>
                <div class="relative">
                    <span class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400">L</span>
                    <input type="number" step="0.1" min="0" max="<?= $remaining_milk ?>"
                ...
                <p class="text-xs text-red-500 mt-1" id="overstockWarning" style="display:none;">
                    <i class="fas fa-exclamation-circle"></i> Cannot sell more than available milk
                </p>
            </div>
...
<!-- ============================================================
     ...
     ============================================================ -->
<script>
const milkPrice = <?= $milk_price ?>;
const remainingMilk = <?= $remaining_milk ?>;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const warningEl = document.getElementById('overstockWarning');
    
    if (litresInput) {
        litresInput.addEventListener('input', function() {
            const litres = parseFloat(this.value) || 0;
            const total = litres * milkPrice;
            
            // Show warning if exceeds available
            if (litres > remainingMilk) {
            ...
            document.getElementById('calculation').textContent = `${litres.toFixed(1)} L × KSh ${milkPrice.toFixed(2)} = KSh ${total.toFixed(2)}`;
        });
        // Trigger on load
        litresInput.dispatchEvent(new Event('input'));
    }
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (litres > remainingMilk) {
                e.preventDefault();
                alert(`Cannot sell ${litres}L. Only ${remainingMilk.toFixed(1)}L available today.`);
                return false;
            }
        });
    }
